"""SQLite persistence adapter (default backend)."""
...
@dataclass
class SQLiteRepository(StructureRepository, FalsificationRepository):
    """Single SQLite-backed implementation of both repository interfaces."""
    ...
    def get_star_system(self, system_id: str) -> Optional[StarSystem]:
        ...
        planets = self.connection.execute(
            "SELECT planet_id, content, confidence FROM planets WHERE system_id = ?",
            (system_id,),
        ).fetchall()
        for planet_id, content, confidence in planets:
            particle_rows = self.connection.execute(
                "SELECT particle_id, weight FROM particles WHERE planet_id = ?",
                (planet_id,),
            ).fetchall()
            particles = [Particle(weight=w, particle_id=pid) for pid, w in particle_rows]
            planet = Planet(content=content, confidence=confidence, particles=particles, planet_id=planet_id)
    ...
    def save_filament(self, filament: Filament, metadata: Optional[dict] = None) -> None:
        ...
        self.connection.execute(
            ...
            (
                ...
                filament.system_a_id,
                filament.system_b_id,
                filament.concept,
                filament.strength,
                md,
            ),
    ...
    def get_filament(self, filament_id: str) -> Optional[Filament]:
        row = self.connection.execute(
            "SELECT system_a_id, system_b_id, concept, strength, filament_id FROM filaments WHERE filament_id = ?",
            (filament_id,),
        ).fetchone()
        if row is None:
            return None
    ...
    def save_galactic_core(self, core: GalacticCore, metadata: Optional[dict] = None) -> None:
        md = json.dumps(metadata or {})
        self.connection.execute("DELETE FROM galactic_core_priors")
        for prior_name, confidence in core.priors.items():
            self.connection.execute(
                "INSERT INTO galactic_core_priors (prior_name, confidence, metadata) VALUES (?, ?, ?)",
    ...
    def load_galactic_core(self) -> GalacticCore:
        ...
        rows = self.connection.execute(
            "SELECT prior_name, confidence FROM galactic_core_priors"
        ).fetchall()
        for prior_name, confidence in rows:
    ...
    def save_captured_fact(self, captured_fact: CapturedFact, metadata: Optional[dict] = None) -> None:
        ...
        self.connection.execute(
            ...
            """,
            (
                captured_fact.planet.planet_id,
                captured_fact.reason,
                captured_fact.captured_at,
                captured_fact.state.value,
                md,
            ),
        )
        self.connection.commit()
    ...
    def record_transition(
        ...
        self.connection.execute(
            """
            INSERT INTO falsification_transitions (planet_id, from_state, to_state, reason, changed_at, metadata)
...
def create_sqlite_repository(db_path: str = ":memory:") -> SQLiteRepository:
    connection = sqlite3.connect(db_path)
    return SQLiteRepository(connection=connection)
